...
def run(args):
    ...
    if FLAGS.use_tpu:
        my_project = subprocess.check_output(['gcloud','config','get-value','project'])
        my_zone = subprocess.check_output([ 'gcloud','config','get-value','compute/zone']) 
        cluster_resolver = tf.contrib.cluster_resolver.TPUClusterResolver( tpu=[FLAGS.tpu_name], zone=my_zone, project=my_project)
    ...
    if(FLAGS.use_tpu):
        estimator = tf.contrib.tpu.TPUEstimator( model_fn=model_fn, config = my_tpu_run_config, use_tpu=True)
    else:
        estimator = tf.contrib.tpu.TPUEstimator( model_fn=model_fn, config=tf.contrib.tpu.RunConfig(), use_tpu=False)
